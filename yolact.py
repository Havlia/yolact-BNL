...
from yolact_submodule.data.config import cfg, mask_type
from yolact_submodule.layers import Detect
from yolact_submodule.layers.interpolate import InterpolateModule
from yolact_submodule.backbone import construct_backbone

import torch.backends.cudnn as cudnn
from yolact_submodule.utils import timer
from yolact_submodule.utils.functions import MovingAverage, make_net
...
if __name__ == '__main__':
    from yolact_submodule.utils.functions import init_console
    init_console()

    # Use the first argument to set the config if you want
    import sys
    if len(sys.argv) > 1:
        from yolact_submodule.data.config import set_cfg
        set_cfg(sys.argv[1])
